...
try {
  const inputs = [
    'https://lite.tiktok.com/t/ZS9STpGp6kK2T-HhDaj/',
    'https://lite.tiktok.com/t/ZS9SXQwDyoQGJ-fqhHA/',
    'https://lite.tiktok.com/t/example-id/?x=a+b&x=a%20b&token=%2f%2F',
  ...
  for (const input of inputs) {
    ...
    if (isTikTokLiteInviteShortLink(input)) {
      assert.equal(result.mode, 'original');
      assert.equal(detectBuildMode(input), 'original');
    }
...
console.log('Official Lite short invitations and OneLinks: opaque IDs and all parameters preserved without network requests');
