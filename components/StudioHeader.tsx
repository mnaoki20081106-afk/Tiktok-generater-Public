...
export function StudioHeader({ publicView = false }: { publicView?: boolean }) {
  return <header className="studio-nav">
    ...
    <nav aria-label="メインナビゲーション">{!publicView && <Link href="/tools/link-generator">リンクツール</Link>}<Link href={publicView ? '/login' : '/dashboard'}>{publicView ? 'はじめる' : 'マイサイト'}<ArrowUpRight size={14} /></Link></nav>
  </header>;
}
