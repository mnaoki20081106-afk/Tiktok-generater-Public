...
function renderLinkCardViewerHtml(d: TemplateData, preview: boolean): string {
  ...
  const hrefJson = JSON.stringify(destination).replace(/</g, '\\u003c');
...
<style>*{box-sizing:border-box}html,body{min-height:100%;margin:0}body{min-height:100dvh;background:#fff;color:#111;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Text','Hiragino Sans','Noto Sans JP',sans-serif;display:grid;place-items:center;padding:24px}.lc-preview{width:min(360px,100%);text-align:center;color:#555}.lc-preview p{font-size:14px;line-height:1.65;margin:0 0 16px}.lc-preview button{appearance:none;border:1px solid #d7d7d7;border-radius:10px;background:#fff;color:#111;font:600 14px/1.2 inherit;padding:12px 16px;cursor:pointer}</style></head><body>${preview ? previewBody : liveBody}${tracking}<script>(function(){window.__linkCardHref=${hrefJson};var msg='TikTokで開きますか？\\nTikTok Liteアプリまたはストアに移動します。';function ask(navigate){if(window.confirm(msg)&&navigate){window.location.href=window.__linkCardHref}}if(${preview ? 'true' : 'false'}){var b=document.getElementById('lc-preview-dialog');if(b)b.addEventListener('click',function(){ask(false)})}else{ask(true)}})();</script></body></html>`;
}
