...
- **遷移は `<a>` のネイティブな挙動に任せる。** `preventDefault()` もしないし `location` も触らない。スクリプトで遷移させるとユーザー操作の文脈から外れ、**Universal Link が発火しなくなる**ため。JS がやるのは「画面を出す」「2秒後にエラー文言を出す」の2つだけ。
- `touchstart` / `click` を拾うのは、タップで遷移が始まったあとにエラー文言が割り込んで出るのを防ぐため（タイマーを止めるだけ）。
- 遷移先は表示直前に `href` へ入れる。抽選で当たりURLに差し替わっている場合に追従するため。
- 公開ページ（クッションOFF）では、フィンガープリント照合（最大1.5秒）の結果を待ってから画面を出す。待ち時間の有無で当選を推測されないようにする仕組みをそのまま保つため。
