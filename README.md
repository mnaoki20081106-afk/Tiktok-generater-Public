...
**通常のブラウザ（Safari など）は、遷移先のURLへそのまま飛ばすだけ。カスタムスキーム（`snssdk473824://`）は使わない。**

一時期これをJSから直接叩いていたが、iOSが「"TikTok Lite"で開きますか？」の確認ダイアログを出し、**そこから開くとアトリビューションが切れる**ことが実機で判明した（招待が無効になる）。公式リンクと同じく素直にURLへ飛ばすのが、トラッキングを保ったままアプリへ入る唯一の経路。

**アプリ内ブラウザ（X など）は自動遷移を一切試みない。**（`IN_APP_BROWSER_PATTERN` / `isInAppBrowser()`）

Xのアプリ内ブラウザはカスタムスキームの起動もページ遷移も両方ブロックし、さらにフォールバック先の公式LPが内部で `onelink.me` へ飛ぼうとしてそこでも止まる。自動遷移を試みるほど手詰まりになるので、**利用者のタップで `<a>` を辿らせて Universal Link を自然に発火させる**。検知対象は X / Facebook / Messenger / Instagram / LINE / WeChat。

画面は真っ白（`#FFFFFF`）で、要素はプログレスバーだけ。テキストもスピナーも出さない。画面全体（`position:fixed; inset:0`）が1枚の `<a>` になっている。

```
1. 真っ白な画面を表示          （0ms）
2. プログレスバーが出現         （PROGRESS_APPEAR_MS = 400ms）
3. 86% まで進んでホールド       （PROGRESS_HOLD_MS = 1400ms → PROGRESS_HOLD_PCT）
4a. タップされたら 450ms で 100% へ（PROGRESS_FINISH_MS）→ <a> が遷移
4b. 2.5秒たってもタップが無ければ「エラーが発生しました。タップして再実行」を表示
    （ERROR_AFTER_MS）→ そこでタップされたら 4a と同じ
```

- **タップ時のアニメーションは視覚的なフィードバックのためだけのもので、遷移は `<a>` のネイティブな挙動に任せる。** `preventDefault()` もしないし `setTimeout` でも飛ばさない。スクリプトで遷移させるとユーザー操作の文脈から外れ、**Universal Link が発火しなくなる**ため。
- `touchstart` と `click` の両方を拾うが、処理は一度だけ（二重発火しても100%アニメを二度描かない）。
- 遷移先は表示直前に `href` へ入れる。抽選で当たりURLに差し替わっている場合に追従するため。
- 公開ページ（クッションOFF）では、フィンガープリント照合（最大1.5秒）の結果を待ってからローディング画面を出す。待ち時間の有無で当選を推測されないようにする仕組みをそのまま保つため。
...
**通常のブラウザではタップを挟まない。** 以前は自動遷移が働かなかった場合の避難口を4.5秒後に表示していたが、遷移自体は始まっているのに遷移先の応答が遅いだけのケースでも出てしまい、実機で「タップして続行」が見えていた。現在はタップ無しで飛ばし、JSが無効な環境向けの `<noscript>` だけ残している(クッションページも同じ)。アプリ内ブラウザだけは自動遷移が効かないため、専用のローディング画面でタップを促す(上記「遷移は環境で2つに分ける」を参照)。
