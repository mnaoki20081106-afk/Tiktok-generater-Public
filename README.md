...
- **2モード**: 単独版と同じく1つのURLで分岐する。`?to=` なしならビルダー(生成フォーム)、`?to=<url>` 付きならクッションページ(2秒前後の遅延後に遷移先へ飛ばす黒画面)。
...
招待LPのURLをただ開かせるだけでは、**Liteがインストール済みでもアプリが起動せず App Store へ飛ばされる**（実機で確認）。原因はiOS側の仕様にある。

- Universal Link は「利用者がリンクをタップしたとき」しか発火しない。こちらのページから `location.replace()` で送り出す遷移はスクリプト起因なので、OSはURLを横取りせず、ただのWebページとしてLPを開く。
- LP側のJSは `inc_target_url` のスキーム起動を試みるが、こちらもユーザー操作を伴わないためSafariにブロックされやすく、LPは所定時間後にストアへフォールバックする。

カスタムスキーム（`snssdk473824://`）への遷移はUniversal Linkと違い**スクリプトからでも実行できる**ので、LPを開く前に一度だけこれを試す（`lib/lite-launch.ts` の `startLiteLaunch()`）。

```
1. location.href = snssdk473824://roma_redirect/?params_url=<招待LP + u_code 等>   ← toLiteAppLink()
2. 1.2秒以内にページが背面へ回らなければ「開かなかった」とみなして LP へ
3. LPへの遷移すらブロックされる環境では、8秒後に手動リンクを表示する
```

- 「アプリが開いたか」は `pagehide` / `blur` / `visibilitychange` でページが背面に回ったかを見る。アプリが前面に出た場合は打ち切り、進行中の起動をWeb遷移で妨害しない。
- スキームだけでは**アプリが起動するだけで「誰の紹介か」が伝わらない**ため、TikTok自身と同じ `params_url` 形式で `u_code` / `share_page_data` などをそのまま載せる。
- **アプリ内WebViewの描画設定を復元する**（`LITE_INAPP_RENDER`）: `__status_bar` / `_pia_` / `_svg` / `enable_canvas` / `enable_canvas_optimize` / `hide_nav_bar` / `should_full_screen` の7件は、Web側では `INTERSTITIAL_PARAMS` として除去している。ブラウザで踏んだときに中間ページ然とした描画を誘発するためで、そこでは不要なもの。一方**アプリ内では招待ページを描画するコンテナ（Spark）の設定そのもの**で、これが無いと「アプリは起動するが招待ページへ画面が切り替わらない」状態になる（実機で確認）。アプリ用ペイロードはWeb用のURLから組み立てる都合で欠落するため、Run A の値のまま補完する。
  - `og_desc_text` / `og_image` / `og_title_text` は同じく除去しているが、こちらは招待LPのOGPカード（SNSでシェアしたときの見た目）用でアプリ内の描画には関与しないため補完しない。
- **アプリ内ブラウザ（X など）では自動遷移を一切行わない**（`IN_APP_BROWSER_PATTERN` / `isInAppBrowser()`）: Xのアプリ内ブラウザはカスタムスキームの起動もページ遷移も両方ブロックし、さらにフォールバック先の公式LPが内部で `onelink.me` へ飛ぼうとしてそこでも止まる。自動遷移を試みるほど手詰まりになるため、UserAgentで検知したら**待たずに手動ボタン（「タップして続行」）だけを出してタップに委ねる**。実タップは Universal Link を発火させられる唯一の手段でもある。検知対象は X / Facebook / Messenger / Instagram / LINE / WeChat。
- **それ以外の環境での手動リンクは既定では出さない。** カスタムスキームもページ遷移も両方ブロックされた場合に限り、8秒たってもページに留まっていれば `hidden` を外す。自動で抜けられた環境では表示されない。以前「遷移先の応答が遅いだけなのに『タップして続行』が出る」問題があったため、表示条件をこの形に絞ってある。
  - 手動リンクの遷移先は表示直前に `opts.webUrl` で更新する。抽選で当たりURLに差し替わっている場合に追従するため。
- クッションページのランダム待機は 1〜2秒 → 0.6〜1.2秒に短縮した。アプリ起動の待ち（1.2秒）が後段に乗るため、合計の体感を従来どおり2秒前後に保つ。
- 抽選で当たりURLに差し替わる場合に備え、`/api/visit` は `href` と一緒に対応する `appLink` も返す。公開ページ側はJSしか持たないため、サーバー側で組み立てないとアプリを直接起動できない。
- `startLiteLaunch()` はクッションページ（React）と公開ページ（文字列HTML）の両方から使う。後者へは `liteLaunchScript()` で関数をそのまま直列化して埋め込むため、**この関数はモジュール内の他の識別子を参照しない**自己完結した実装にしてある。
