...
#### 遷移先は招待LPのURL。基準は「マージ#36 の出力」

実機で「タップ1回でアプリが起動し、かつ『自身を招待できません』が出る（＝トラッキング成立）」
状態にあったのは**マージ#36** の時点。当時の生成器を実際に走らせて出力を突き合わせてあり、
現在の `buildLpUrl()` の出力は**それと1バイトも違わない**。

公式の招待LPのURLとの差分は次の2点（計11箇所）。**両方あって成立していた。**
