...
- **手動リンクは既定では出さない。** Xのアプリ内ブラウザなどカスタムスキームもページ遷移も両方ブロックされる環境で、8秒たってもページに留まっている場合だけ `hidden` を外す。自動で抜けられた環境では表示されない。以前「遷移先の応答が遅いだけなのに『タップして続行』が出る」問題があったため、表示条件をこの形に絞ってある。
