...
3. `.env.local.example` を `.env.local` にコピーし、SupabaseのURL/anonキー・Service Roleキー・管理者メールアドレス(`ADMIN_EMAILS`)・IPハッシュ鍵(`IP_HASH_SECRET`)を設定する。YouTube風モードの人気動画自動入力を使う場合は、YouTube Data API v3のサーバー用キー(`YOUTUBE_API_KEY`)も設定する(`.env.local` は `.gitignore` 済みなのでリポジトリにはコミットされません)。
   ...
   ```
   本番では同じ値をホスティング先のランタイム環境変数にも設定します。GitHub Actionsを使わずにVercel等がリポジトリを直接デプロイする構成では、GitHub Secretsへ追加するだけではアプリへ渡らないため、ホスティング側にも設定が必要です。
...
6. `ADMIN_EMAILS` に指定したアカウントでログインすると `/admin` の管理画面から利用状況、X監視キーワード、サプライズ抽選を管理できる。X監視キーワードの書き込みにはサーバー専用の `X_BUNSEKI_GITHUB_TOKEN` を使用する。
...
| `/admin` | 管理画面。利用状況・X監視キーワード・サプライズ抽選を管理する。`ADMIN_EMAILS` のアカウントのみアクセス可 |
...
- **公開ページのリンクは、サーバー側で `href` を入れた状態で最初から出す。** フィンガープリント照合は裏で走らせ、当たりURLが本人向けに差し替わったときだけ `href` を書き換える。クリックを `preventDefault()` で横取りせず、Universal Linkを利用者のタップで辿らせる。
- `startLiteLaunch()` は単独リンクジェネレーターのクッションページで使う。`liteLaunchScript()` で文字列HTMLにも埋め込めるよう、**モジュール内の他の識別子を参照しない**自己完結した実装にしてある。
...
- **Refererの匿名化**: 遷移先へこのサイトのドメインをRefererとして渡さない。リンクジェネレーターのクッションページと各公開ページの `<head>` に `<meta name="referrer" content="no-referrer">` を入れ、リンクにも `rel="noreferrer noopener"` を付けている。
...
サイト編集画面(`/dashboard/[id]`)では、各作成モードの公開ページを常に表示する。従来の「クッションページを挟む」ON/OFFは廃止し、`content_data.useCushionPage` も新規保存しない。既存データにこの値が残っていても公開処理では参照しない。

「リンクカード」モードは、画像を使わない白背景の確認画面を表示する。「TikTokで開きますか？」ダイアログの「TikTokで開く」は通常の `<a>` として実装し、保存時に取得したTikTok公式のアプリ／ストア分岐URLをそのまま `href` に設定する。背景画像とプロフィール画像は公開画面に表示せず、背景画像は必須項目から外す。OGP画像はSNS上のリンクカード表示に使うため引き続き必須。

それ以外のニュース風・Instagram風・Instagramライブ風・ライブ配信風・X風・TikTok風・YouTube風・ファイル共有風は、選択したレイアウトを表示し、その中のボタンまたはタップ領域から招待リンクへ進む。

保存時には作成モードに関係なく `generateDestinationUrl()` を適用する。TikTok Lite公式短縮URLの場合、招待LPに描画された公式 `lite_redirect` を優先して保存し、アプリ起動用の `redirect_url` と未インストール時の `short_dl` を保持する。公式分岐情報を取得できない場合は、検証済みの `lite.tiktok.com/t/...` を保持して公開を継続する。

#### 当たりURL

抽選の当たりURLも公開ページと同じ `generateDestinationUrl()` を保存時に使う。既存DBとの互換性のため `prize_url_optimized` 列は維持するが、管理画面では「URL最適化」という名称や形式判定を表示しない。解決済みURLがない旧設定は `prize_url` にフォールバックする。
