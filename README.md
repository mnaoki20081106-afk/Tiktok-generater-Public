...
実機で次の4通りを確認した結果、成立するのは1つだけだった。

| 経路 | アプリ起動 | トラッキング |
|---|---|---|
| **招待LPのURL → そのページからアプリへ** | LP経由 | **○**（「自身を招待できません」が出る） |
| カスタムスキーム（マージ#35 と1バイト一致） | ○ | × |
| カスタムスキーム（公式HTMLの `url_schemes` と1バイト一致） | ○ | × |
| OneLinkラッパー（公式HTMLの `url_fallback` と同形） | ○ | × |

カスタムスキームは**2種類をそれぞれバイト単位で再現**して試した（当時のコミットから
`lib/link-generator.ts` を取り出し、出力同士を突き合わせて `params_url` のキー数・値まで
一致を確認したうえで実機テスト）。どちらもアプリは開くがトラッキングは落ちた。
**ペイロードの中身の問題ではなく、LPのページを経由するかどうかが分かれ目だった。**

> アプリを開く役目はLP自身のJS（`incentive_redirect=1` / `is_inc_roma=1` / `inc_target_url`）に任せる。
> こちらの仕事は、黒画面のタップ誘導で利用者を確実にLPへ送り届けるところまで。
> **ワンクリックでアプリを直接開く形は、トラッキングと両立しない。**

タップ数は公式の招待リンクと同じ（リンクを踏む → LPで進む）。公式と同等が上限で、
それ以上に短縮するとトラッキングが失われる。
