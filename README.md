...
- **未インストール端末の逃がし先を持つ（`storeFallbackUrl` / `APP_FALLBACK_MS`）。** カスタムスキームを踏んでもアプリが無ければ何も起きず、黒画面に取り残される。タップから2.5秒たっても画面が見えたままなら https のラッパーへ送り、AppsFlyer にストアへ振り分けさせる（ディファードディープリンクで招待も引き継がれる）。**タップ自体には介入していない**（`preventDefault()` もしないし `href` も触らない）。ネイティブの遷移が始まらなかった場合にだけ動く後処理で、アプリが開いていればページが `hidden` になるので何もしない。
